"""
COSC 4368 AI — Check-in 3 | maze_3_3.py
may need to chnage the name to maze_solver.py to run with main.py
...
# IMPORTANT FIX: keep mazes in same folder as this file
MAZE_ROOT = BASE_DIR
...
class MazeEnvironment:
    ...
    def __init__(self, maze_id: str):
        ...
        # 🔍 DEBUG — ADD THIS
        print("\n--- DEBUG HAZARDS ---")
        print("Fire pivots:", len(self.fire_pivots))
        print("Teleport pairs:", len(self.teleport_pairs))
        print("Cell types count:", len(self.cell_types))
        print("---------------------\n")
    ...
    def _move(self, pos: Tuple[int,int], act: Action) -> Tuple[Tuple[int,int], bool]:
        ...
        # IMPORTANT:
        # Do NOT block FIRE here.
        # Fire is handled later via _is_deadly() (timing-based hazard)
...
class DynaQAgent:
    ...
    def boot(self, env: MazeEnvironment):
        """Load environment structure and prepare model-based planning."""
        ...
        # Load all walls from the environment
        for x in range(NUM_CELLS):
            for y in range(NUM_CELLS):
                for dx, dy in DIRS4:
                    nx, ny = x + dx, y + dy
        ...
        # Time-aware model-based planner:
        # This finds a route using position + fire phase + confusion state.
    ...
    def _bfs(self, start, goal, allow_unknown=True, danger_thresh=0.9):
        ...
        while q:
            ...
            for nxt in self._neighbors.get(pos, []):
                ...
                cell_penalty = self.danger.get(nxt, 0)

                # Prefer avoiding visible fire cells,
                # but do NOT make them impossible like walls.
                if hasattr(self, "fire_cells") and nxt in self.fire_cells:
                    cell_penalty += 1.2

                if cell_penalty > danger_thresh:
                    continue
    ...
    def plan_turn(self, res: Optional[TurnResult]) -> List[Action]:
        ...
        # Keep this disabled if you want real exploration/learning behavior.
        # Scripted actions make the stats look artificially perfect.
        ...
        # Fire timing safety:
        # The agent does NOT own self.fire_pivots, so do not use it here.
        # Instead, use fire_adj/death_cells, which the agent already tracks.
        if (
            len(self._plan) >= 2
